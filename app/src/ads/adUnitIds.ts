...
type Placement = "produceDetail" | "history" | "favorites" | "settings" | "removeAds" | "residue" | "pesticideInformation";
...
const REAL_BANNER_UNIT_IDS: Record<Placement, string | undefined> = {
  ...
  residue: Platform.select({
    ios: process.env.EXPO_PUBLIC_ADMOB_IOS_BANNER_RESIDUE,
    android: process.env.EXPO_PUBLIC_ADMOB_ANDROID_BANNER_RESIDUE,
  }),
  pesticideInformation: Platform.select({
    ios: process.env.EXPO_PUBLIC_ADMOB_IOS_BANNER_PESTICIDE,
    android: process.env.EXPO_PUBLIC_ADMOB_ANDROID_BANNER_PESTICIDE,
  }),
